...
- **Comms → Discord (NEXT QUEUED — user 2026-08-07, promoted over task dashboard)** — vision v6 "v2 comms integration for chat delivery"; currently curl stubs (`discord_webhook`/`slack_webhook`) in canonical `forkable/scripts/report_sink.sh`. Goal: everything the agent produces (weekly review, session close-outs, screenshot PR snippets, error reports) delivered into a Discord channel → removes the need for a separate task-dashboard app. Design delegated to @architect (`docs/superpowers/specs/2026-08-07-comms-discord-design.md`), implementation delegated to a subagent per execution-model protocol. Forkable-first: canonical changes in forkable, pull to khelam (tripwire-enforced).
