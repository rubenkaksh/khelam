...
- **Real auth/JWT wiring — CLOSED 2026-08-07** — bearer token flow confirmed wired: `auth_api_service.dart:107,143` call `DioApiClient.setBearerToken(session.accessToken)` on login/restore; `booking_flow` integration test booked a live slot 2/2 on iOS sim (no 401). Was: `POST /slots/:id/book` returned 401 until a bearer token was sent via `DioApiClient.setBearerToken`.
...
- **UI screenshot verification — CLOSED 2026-08-07 (superseded by OA#6)** — implemented forkable-first as `capture_screens.sh` + `screens.yaml` schedule registry (`ec178eb`, `b815d6c`); screenshots land in `~/projects/sandbox/screenshots/<date>`. The open questions below are resolved in that design; keep here only as reference. (Original idea: a separate package/script — `khelam_screenshot_verify` or similar — that boots the app, navigates the flow, and saves screenshots to a timestamped dir for the review/closeout.)

## From the 2026-08-07 session

- **Task dashboard — DROPPED 2026-08-08 (user: "we won't be picking it")** — was PARKED 08-07; the 08-16 DROP criterion fired early: Discord comms (digest + weekly reviews) cover the review surface. **Spec RETAINED for revival** at `docs/superpowers/specs/2026-08-06-task-dashboard-design.md` (standalone Flutter + Supabase queue, est ~28–34k, decisions 1–10 locked). Reopen = user decides to revisit; pick up from the locked design, no re-design needed.
- **macOS DMG verification — PARKED (user 2026-08-07, blank screen)** — the release DMG app (`build/macos/khelam-1.0.0.dmg`, ad-hoc signed) launches to a **blank screen**. Descoped with T4/T6; **revisit at the 2026-08-16 review**: diagnose (possible causes: window/content not rendering under release entitlements, or a shared-flow regression) or drop macOS distribution entirely.
- **Comms → Discord — IMPLEMENTED (2026-08-07)**: v1 single-webhook delivery shipped forkable-first (forkable `f771d0c`, khelam `b815d6c`) + v2 multi-channel cross-project hub locked via Grill Gate (12/12 answers + Q3b; forkable `1e4c942`, khelam `e0f59bf` + spec v2 `8cab2a4`). Channels: `#daily-overview` (new `daily_digest.sh` @ 08:00 Mon–Fri, launchd template `com.khelam.daily-digest.plist`), `#weekly-reviews`, `#screenshots`, `#agent-errors`; shared env `~/.config/opencode/discord.env` (v1 alias `DISCORD_WEBHOOK_URL` kept). **GO-LIVE STEPS — PARKED (user 2026-08-07, on hold)**: (1) create `~/.config/opencode/discord.env` with `DISCORD_WEBHOOK_URL` + `DAILY_OVERVIEW_WEBHOOK_URL`/`WEEKLY_REVIEWS_WEBHOOK_URL`/`SCREENSHOTS_WEBHOOK_URL`/`AGENT_ERRORS_WEBHOOK_URL`; (2) install digest job: `cp scripts/com.khelam.daily-digest.plist ~/Library/LaunchAgents/` + `launchctl load`; (3) live E2E (digest post, weekly report, screenshot → #screenshots, forced error → #agent-errors); (4) optional: forkable weekly-review arm (plist instance, rename). **Requirement (user 2026-08-07): the Discord comms must mirror the `docs/tasklog.md` board structure** — daily digest + weekly review surface the live task board (statuses: 🔴 Active / 🟡 Backlog / 🟠 Parked / ⚪ Descoped / ⚠️ User actions / 📋 Closed) so the channels read like the Trello board, not a raw report dump. Resume when the user wants to go live; after go-live, the task-dashboard PARKED item above gets its 2026-08-16 decision — DROP if the channels cover the review surface, else re-estimate.
...
## From the 2026-08-07 startup-architecture audit (T-tasklist)

Audit source: Reddit "10 most expensive Flutter mistakes" (r/FlutterDev 1u244sp) → @architect read-only audit at `~/projects/sandbox/tmp/startup-architecture-audit-2026-08-07.md`. Placement per OA#5: commons-first (version bump → both forks pin) / forkable-first (shared app shell) / khelam-only (feature-specific). Priorities set 2026-08-07; order per urgency.

- **[T1] Security gate: `.env` de-tracked — DONE 2026-08-07** — `.env` added to khelam `.gitignore` (commit `9445e2d`) + forkable canonical `.gitignore` (commit `c067908`, children inherit). Removed from git index; working file kept (pubspec asset). **USER ACTION (flag):** rotate the leaked Google **SERVER client ID** on Google Cloud Console (optional `filter-repo` history purge later).
- **[T2] Shared error mapping — NEXT (pick today/tomorrow)** — Commons `AppException` hierarchy + `DioApiClient` mapping (4xx/5xx/timeout/offline → typed user messages); `BookingApiService` consumes it (drop `catch (_)` discards in `schedule_cubit.dart:95,121,156` + `turf_selection_cubit.dart:78`); add `EmptyView` for zero slots (`schedule_view.dart:71-130`). M effort, commons-first → khelam-only. Auth already proves the pattern.
- **[T3] Retry + connectivity-aware network — after T2** — Commons `RetryInterceptor` (1–2 attempts, backoff) on `DioApiClient.raw` + connectivity gate → surface "offline"/"unavailable" instead of a single timeout hard-fail, and replace the masking fallback in `TurfsApiRepository` (any error → stale dummy turfs). M effort, commons-first.
- **[T5] In-session GET cache — after T3** — Commons `DioApiClient` GET cache (recent responses, ~30s TTL) so re-selecting a viewed date is served from memory (`schedule_cubit.dart:110-113`). M effort, commons-first. Full offline cache + booking queue = L item, deferred to backlog (matches README "Out: persistence/offline").
- **[T4] Analytics — PARKED (user 2026-08-07, was DESCOPED)** — no analytics seam, no event triggers, until analytics is actually planned. **Revisit at the 2026-08-16 review** (same date as the task dashboard): either plan it properly or drop. (Samseer is a dev network inspector, not analytics.)
- **[T6] Perf/rebuild hygiene + Samseer-in-prod gate — PARKED (user 2026-08-07, was DESCOPED)** — bundled with T4. **Revisit at the 2026-08-16 review** (same date as the task dashboard); re-add when needed.
