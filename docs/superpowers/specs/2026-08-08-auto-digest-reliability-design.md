...
- **CONTENT CHANGE — single TRIMMED message, final layout (user 2026-08-08, supersedes the 4-message split)**: one message = one curated board. Cards TRIMMED to *italic title* + subtitle + **short status** + date (scope/effort/long notes dropped — awk `render_tasklog`); 🔴 Active (all) + 🟡 Backlog (**top 5** only); 🟠 Parked + ⚪ Descoped render only when Active AND Backlog are both empty; blank-line section separators (no `---` dividers — Discord renders them as plain text); full board (Closed, exact tables) stays in the repo (agent console). **No `.md` attachment** — Discord shows it as a download chip, not glanceable; the artifact arg was removed from `send_with_retry` (empty-title branch stays for other callers). Body ≈ 726 chars vs the 2000 cap → the 1990-char truncation guard is now a backstop, not a normal constraint. Marker scheme unchanged (`<date>-1` — backward-compatible with the per-message era; all 48 `-1` markers exist, no re-sends).
- **Verified live (single message)**: render harness on the real board = 726 chars, Active 1 card + Backlog top 5 exactly, Parked/Descoped suppressed, dates/statuses trimmed; kickstart fired 1 real single-format Discord post for 08-06 (14:37, 0 artifacts), marker `2026-08-06-1.sent` rewritten, idempotent re-fire 0 sent / 14 skipped. First scheduled trimmed fire Mon 08-10 08:00.
