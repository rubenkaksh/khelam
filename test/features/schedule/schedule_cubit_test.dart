...
import 'package:khelam/data/repositories/booking_repository.dart';
...
import 'package:khelam/domain/models/turf_summary.dart';
import 'package:khelam/domain/repositories/booking_service.dart' as svc;
import 'package:khelam/ui/features/schedule/bloc/schedule_cubit.dart';
...
void main() {
  late FakeBookingRepository mockRepo;

  setUp(() {
    mockRepo = FakeBookingRepository();
  });
  ...
  group('ScheduleCubit', () {
    ...
    test('selectDate refreshes slots for the new date', () async {
      ...
      final DateTime initialDate = cubit.state.selectedDate!;
      final DateTime nextDate = initialDate.add(const Duration(days: 1));
    ...
    test('selectDate keeps existing turf when turf is already loaded', () async {
      final cubit = ScheduleCubit(repository: mockRepo);
      await cubit.load();

      final DateTime nextDate = cubit.state.selectedDate!.add(
    ...
    test('dayStats computes booking count and revenue', () async {
      ...
      final ScheduleSlotItem availableItem = ScheduleSlotItem(slot: availableSlot);

      final repo = BookingRepository(
        service: _TestBookingService(mockRepo, [bookedItem, availableItem]),
      );
      final testCubit = ScheduleCubit(repository: repo);
      await testCubit.load();
...
class _TestBookingService implements svc.BookingService {
  _TestBookingService(this.fallback, this.slots);

  final FakeBookingRepository fallback;
  final List<ScheduleSlotItem> slots;

  @override
  Future<TurfSummary> getTurf(String turfId) async {
    return fallback.getTurf(turfId);
  }
