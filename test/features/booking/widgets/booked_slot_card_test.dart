...
import 'package:khelam/features/booking/widgets/booked_slot_card.dart';
...
void main() {
  testWidgets('BookedSlotCard shows customer name when provided', (
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: Scaffold(body: BookedSlotCard(item: item)),
      ),
    );
  ...
  testWidgets('BookedSlotCard fires onTap when tapped', (
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: Scaffold(
          body: BookedSlotCard(item: item, onTap: () => tapped = true),
        ),
      ),
    );
