...
void main() {
  group('DioApiClient', () {
    ...
    test('getJsonList sends query parameters and returns a list of objects',
        () async {
    ...
    test('getJsonList throws ApiClientException when response is not a list',
        () async {
      final Dio dio = Dio()
        ..httpClientAdapter = _StaticJsonAdapter(<String, dynamic>{
          'status': 'ok',
    ...
    test('postJson sends the body and returns the JSON object response',
        () async {
      final _RecordingJsonAdapter adapter = _RecordingJsonAdapter(
        (RequestOptions options) async {
          expect(options.method, 'POST');
          expect(options.path, '/slots/s1/book');
          expect(options.data, <String, dynamic>{'customerPhone': '9876543210'});
