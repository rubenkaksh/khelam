...
class MockBookingService implements BookingService {
  ...
  Future<List<ScheduleSlotItem>> getSchedule({
    ...
    for (int hour = _openHour; hour < _closeHour; hour++) {
      ...
      final DateTime endTime = normalizedDate.add(
        Duration(hours: hour + 1),
      );

      final String sid = _slotId(hour, date);
      final bool isBooked = _isSlotBooked(hour, date) || _extraBookedSlotIds.contains(sid);
      ...
      if (isBooked) {
        ...
        booking = Booking(
          ...
          bookingCode: 'BK-${date.month.toString().padLeft(2, '0')}${date.day.toString().padLeft(2, '0')}-${hour.toString().padLeft(2, '0')}',
          userId: 'user-1',
          turfId: _turfId,
          slotId: sid,
          totalAmount: 100.0,
